...
export function SchemaViewer({ schema, workspaceId, conversationId, onSync, dbType }: SchemaViewerProps) {
  ...
  return (
    <div className="h-full flex flex-col transition-all duration-500 ease-in-out">
      ...
      <div className="flex-shrink-0 bg-surface dark:bg-background-dark border-b border-border-DEFAULT dark:border-border-dark p-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-s text-text-muted-DEFAULT dark:text-text-muted-dark">
            {schemaDef?.tables?.length || 0} {(schemaDef?.tables?.length || 0) === 1 ? 'table' : 'tables'}
          </p>

          {/* Action Buttons */}
          <div className="flex items-center gap-2">
            {workspaceId && dbType !== 'simulation' && (
              <button
                onClick={handleSync}
                disabled={isSyncing}
                className={cn(
      ...
      {/* Content */}
      {!schemaDef || !schemaDef.tables || schemaDef.tables.length === 0 ? (
        <div className="flex-1 overflow-y-auto p-2">
          <div className="flex flex-col items-center justify-center h-full p-8 text-center">
            ...
            <p className="text-sm text-text-muted-DEFAULT dark:text-text-muted-dark max-w-sm mb-4">
