...
async def get_database_schema(db = Depends(get_db) if settings.ENABLE_DATABASE else None):
    """
    Get complete database schema information
    
    Returns tables, columns, indexes, foreign keys, etc.
    """
...
async def get_table_sample(
    ...
    try:
        sample = await inspector_service.get_table_sample(db, table_name, limit)
