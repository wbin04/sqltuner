...
export function useEditorLogic({ connectionId }: UseEditorLogicProps) {
  ...
  const handleOptimize = useCallback(
    async (sql: string) => {
      // If no active conversation, create one first (to continue in same chat)
      if (!activeConversationId) {
        const response = await sendMessageMutation.mutateAsync(
          `Optimize query: ${sql.substring(0, 50)}...`
        );
    ...
    [optimizeSqlMutation, activeConversationId, sendMessageMutation]
  );
